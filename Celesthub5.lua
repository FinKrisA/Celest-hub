...
SettingsTab:Button({
    ...
    if remote then remote = remote:FindFirstChild("FromLocalForServer") end
    if remote then remote = remote:FindFirstChild("Code") end
    if remote then remote = remote:FindFirstChild("EnterCode") end

    if not remote then
        print("Erreur: Remote introuvable")
        return
    end

    -- Liste des codes (Mets les codes actuels ici)
    local codes = {
        "RELEASE",
        "UPDATE",
        "TRADESTRENGTH",
